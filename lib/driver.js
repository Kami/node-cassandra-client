...
Connection.prototype._killPendingQueries = function(err) {
  ...
  for (key in this._pendingQueryCallbacks) {
    if (this._pendingQueryCallbacks.hasOwnProperty(key)) {
      callback = this._pendingQueryCallbacks[key];
      callback(err);
    }
  }
};
...
Connection.prototype._getQueryId = function() {
  // If there are more than 500k outgoing queries at any point in time we have
  // different issues :)
  var maxId = 500000;

  this._queryIdCounter++;

  if (this._queryIdCounter > maxId) {
    this._queryIdCounter = 0;
  }
...
Connection.prototype.execute = function(query, args, callback) {
  ...
  var longQueryWatch = setTimeout(function() {
    self.emit('log', 'trace', 'query is taking long: ' + cqlString, {'query': cqlString, 'timeout': queryWatchTimeout});
  }, queryWatchTimeout);
  var timeoutId = setTimeout(function() {
  ...
  self.client.execute_cql_query(cql, ttypes.Compression.NONE, function(err, res) {
    ...
    delete self._pendingQueryCallbacks[queryId];

    if (!timeoutId) {
      self.emit('log', 'warn', 'query returned after timeout: ' + cqlString, {'query': cqlString, 'timeout': 10000});
      return;
    } else {
      clearTimeout(timeoutId);
    }
