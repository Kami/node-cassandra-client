    ...
    create  : function(callback) {
      ...
      var tries = self.nodes.length;
	    ...
	    function retry(curNode) {
	      ...
	      var conn = new Connection(node[0], node[1], config.keyspace, config.user, config.pass, {use_bigints: self.use_bigints});
 ...
 * @param keyspace
 */
Connection = module.exports.Connection = function(host, port, keyspace, user, pass, options) {
    ...
    this.client.execute_cql_query(cql, ttypes.Compression.NONE, function(err, res) {
      if (err) {
        ...
        if (res.type === ttypes.CqlResultType.ROWS) {
          ...
          var decoder = new Decoder(self.validators[cfName], {use_bigints: self.options['use_bigints']});
