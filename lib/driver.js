  ...
  this.connector = function() {
    ...
    if (self.isConnecting) {
      ...
      tcon.on('error', function(err) {
        log.error('ERR_ON_CONNECT ' + host + ':' + port);
        log.error(err);
        self.isClosed = true;
...
PooledConnection = module.exports.PooledConnection = function(hosts, keyspace, user, pass) {
  this.nodes = [];
  this.current_node = 0;
  ...
  for (var i = 0; i < hosts.length; i++) {
    hostSpec = hosts[i];
    console.log("working on " + hostSpec);
  ...
  this.pool = genericPool.Pool({
    name    : 'Connection',
	  create  : function(callback) {
	    ...
	    var node = self.nodes[self.current_node];
	    callback(new Connection(user, pass, node[0], node[1], keyspace));
	  },
	  destroy : function(conn) { conn.close(); },
	  max     : 10,                 // FIXME: this should be adjustable
	  idleTimeoutMillis : 30000,    // FIXME: this should be adjustable
	  log : true
  });
};
...
PooledConnection.prototype.execute = function(query, args, callback) {
  var self = this;
  this.pool.acquire(function(conn) {
...
PooledConnection.prototype.shutdown = function() {
  var self = this;
    ...
    if (!self.connection.isConnected) {
      ...
      while (self.q.size() > 0) {
        ...
        if (self.connection.isConnected) {
          ...
          log.error('connection is buggered');
        }
      }
    }
  });
