 ...
 */
exports.bind = function(query, args) {
  if (args.length === 0) {
    return query;
  }
  var q = 0;
...
Connection.prototype.execute = function(query, args, callback) {
  var cql = cql = exports.bind(query, args),
  ...
  if (cql === nullBindError) {
    ...
    var timeoutId = setTimeout(function() {
      callback(new Error('Connection timed out'), null, metadata);
      timeoutId = null;
    }, self.query_timeout);
...
var PooledConnection = module.exports.PooledConnection = function(config) {
  ...
  this.query_timeout = config.query_timeout || DEFAULT_QUERY_TIMEOUTl
...
PooledConnection.prototype.execute = function(query, args, executeCallback) {
  ...
  self._getNextCon(function(err, con) {
    ...
    try {
      ...
      self.emit('log', 'error', 'Error while executing a query: ' + err.toString(), {'err': err});
