...
Connection.prototype.execute = function(query, args, callback) {
  ...
  self.client.execute_cql_query(cql, ttypes.Compression.NONE, function(err, res) {
    ...
    if (!timeoutId) {
      self.emit('log', 'warn', 'query returned after timeout: ' + cqlString, {'query': cqlString, 'timeout': self.query_timeout});
      return;
    } else {
      clearTimeout(timeoutId);
    }
    ...
    if (err) {
      ...
      if (res.type === ttypes.CqlResultType.ROWS) {
        ...
        // todo: complain hysterically if self.validators[cfName] is falsey.
